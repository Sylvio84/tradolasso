...
import { Typography, Table, Card, Space, Button, Modal, Form, Input, InputNumber, Select, DatePicker, App, Alert } from "antd";
...
import { AssetNotesModal } from "../asset-notes";
...
export const WalletDetailsPanel = ({ walletId }: WalletDetailsPanelProps = {}) => {
  ...
  const { getRate: getCurrentRate, loading: ratesLoading, error: ratesError } = useCurrencyRates();

  const {
    query: { data, isLoading },
  } = useShow(
    walletId !== undefined
      ? { resource: "wallets", id: walletId }
      : {}
  );

  const record = data?.data;
  const actualWalletId = walletId ?? record?.id;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setEditingLine(null);
      invalidate({ resource: "wallets", invalidates: ["detail"], id: actualWalletId });
    } catch (error) {
  ...
  const handleSaveTransaction = async () => {
    ...
    try {
      ...
      await http(`/wallets/${actualWalletId}/transactions`, {
        method: "POST",
  ...
  return (
    <div style={{ padding: "24px" }}>
      {ratesError && (
        <Alert
          message="Impossible de charger les taux de change actuels. Utilisation des taux historiques."
          type="warning"
          closable
          ...
          <Table
            ...
            summary={() => {
              ...
              lines.forEach((line) => {
                ...
                const buyRate = parseFloat(line.buyConversionRate || "1");
                const currentRate = getCurrentRate(line.currency);

                totalBuyEur += quantity * buyPrice * buyRate;
                totalCurrentEur += quantity * lastPrice * currentRate;
              });
          ...
          >
            ...
            <Table.Column
              ...
              render={(_, record: WalletLine) => {
                ...
                const rate = getCurrentRate(record.currency);
                const priceEur = price * rate;
                const isEur = record.currency === "EUR" || !record.currency;
                return (
                  <div>
            ...
            <Table.Column
              ...
              render={(_, record: WalletLine) => {
                ...
                const buyRate = parseFloat(record.buyConversionRate || "1");
                const currentRate = getCurrentRate(record.currency);
                const totalBuy = quantity * buyPrice * buyRate;
                const totalCurrent = quantity * lastPrice * currentRate;
            ...
            <Table.Column
              ...
              render={(_, record: WalletLine) => (
                <Space>
                  {record.asset?.id && (
                    <>
                      <Link to={`/assets/show/${record.asset.id}?from=wallet:${actualWalletId}`}>
                        <Button
                          type="text"
                          size="small"
                          icon={<EyeOutlined />}
...
export { WalletDetailsPanel as default };
